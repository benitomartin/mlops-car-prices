...
@task
def clean_data(data_directory_path:str) -> None:
...
def split_dataframe(df: pd.DataFrame) -> tuple:
    ...
    X = df.drop(columns=["price"])  # DataFrame containing feature columns (all columns except 'price')
    y = df["price"]                  # Series containing the target variable column ('price')
...
@task
def prep_db(create_table_query, sorted_results):
    ...
    create_table_query = text(create_table_query.strip())
    ...
    try:
        ...
        sorted_results.to_sql('evaluation_results', con=engine, if_exists='append', index=False)
    finally:
        engine.dispose()
